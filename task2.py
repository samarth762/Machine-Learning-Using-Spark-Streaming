...
def main():
	...
	def readMyStream(rdd):
		if not rdd.isEmpty():
			...
			print(acc1,"rf")
			y_t=testData.select("label").collect()
			y_p=predictions.select("prediction").collect()
			print(sklearn.metrics.confusion_matrix(y_t, y_p),"conf_mat_rf")
			print(sklearn.metrics.classification_report(y_t,y_p,target_names=['spam','ham']))
			...
			print(acc2,"svm")
			y_p=predictions.select("prediction").collect()
			print(sklearn.metrics.confusion_matrix(y_t, y_p),"conf_mat_svm")
			print(sklearn.metrics.classification_report(y_t,y_p,target_names=['spam','ham']))
			...
			y_p=predictions.select("prediction").collect()
			print(sklearn.metrics.confusion_matrix(y_t, y_p),"conf_mat_nav_b")
			print(sklearn.metrics.classification_report(y_t,y_p,target_names=['spam','ham']))
			print(acc3,"nb")
			"""#def preprosses(a):
				#up_l=[]
